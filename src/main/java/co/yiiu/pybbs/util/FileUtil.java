...
public class FileUtil {
  ...
  public String upload(MultipartFile file, String fileName, String customPath) {
    try {
      ...
      File savePath =
          new File(
              systemConfigService.selectAllConfig().get("upload_path").toString() + customPath);
      if (!savePath.exists()) savePath.mkdirs();

      // 给上传的路径拼上文件名与后缀
      String localPath =
          systemConfigService.selectAllConfig().get("upload_path").toString()
              + customPath
              + "/"
              + fileName
              + suffix;
      ...
      // 上传成功后返回访问路径
      return systemConfigService.selectAllConfig().get("static_url")
          + customPath
          + "/"
          + fileName
          + suffix
          + "?v="
          + StringUtil.randomNumber(6);
    } catch (IOException e) {
   ...
   * @param customPath 上传到oss里的目录地址，如果只传到bucket下，请传空字符串"" 例: 现在想将file放在 2021-03-08/test/avatar
   *     目录里，那么这里的customPath就传 2021-03-08/test/avatar
   * @return
   */
  ...
  public String generatorOssUrl(String name) {
    ...
    GeneratePresignedUrlRequest generatePresignedUrlRequest =
        new GeneratePresignedUrlRequest(ossBucket, name);
  ...
  public String qiniuUpload(MultipartFile file, String fileName, String customPath) {
    try {
      ...
      // response.bodyString():
      // {"hash":"FrvhXY3VZrmU6_vUYLdQtk1KKlUH","key":"FrvhXY3VZrmU6_vUYLdQtk1KKlUH"}
