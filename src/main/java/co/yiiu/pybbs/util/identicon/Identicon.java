...
public class Identicon {
  ...
  public String saveFile(String username, BufferedImage image) {
    ...
    try {
      File file =
          new File(
              systemConfigService.selectAllConfig().get("upload_path").toString() + userAvatarPath);
      ...
      return systemConfigService.selectAllConfig().get("static_url").toString()
          + userAvatarPath
          + fileName;
    } catch (IOException e) {
  ...
  //      DefaultPutRet defaultPutRet = JsonUtil.jsonToObject(response.bodyString(),
  // DefaultPutRet.class);
