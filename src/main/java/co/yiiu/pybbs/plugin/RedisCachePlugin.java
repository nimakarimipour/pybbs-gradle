...
public class RedisCachePlugin {
  ...
  @Resource private ITopicService topicService;
  @Resource private ISystemConfigService systemConfigService;
  ...
  public Object topicSelectById(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    String topicJson =
        redisService.getString(
            String.format(RedisKeys.REDIS_TOPIC_KEY, proceedingJoinPoint.getArgs()[0]));
    if (topicJson == null) {
      Object topic = proceedingJoinPoint.proceed(proceedingJoinPoint.getArgs());
      // 缓存在redis里
      redisService.setString(
          String.format(RedisKeys.REDIS_TOPIC_KEY, proceedingJoinPoint.getArgs()[0]),
          JsonUtil.objectToJson(topic));
      return topic;
    } else {
  ...
  public Object topicUpdateViewCount(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    String s =
        redisService.getString(
            String.format(RedisKeys.REDIS_TOPIC_VIEW_IP_ID_KEY, ip, topic.getId()));
      ...
      if (s == null) {
        ...
        redisService.setString(
            ...
            Integer.parseInt(
                systemConfigService
                    .selectAllConfig()
                    .get("topic_view_increase_interval")
                    .toString()));
      }
      return topic;
    } else {
  ...
  public void topicUpdate(JoinPoint joinPoint) {
    ...
    // 缓存到redis里
    redisService.setString(
        String.format(RedisKeys.REDIS_TOPIC_KEY, topic.getId()), JsonUtil.objectToJson(topic));
  }
  ...
  public Object commentSelectByTopicId(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    String commentsJson =
        redisService.getString(String.format(RedisKeys.REDIS_COMMENTS_KEY, topicId));
    if (commentsJson != null) {
      ...
      redisService.setString(
          String.format(RedisKeys.REDIS_COMMENTS_KEY, topicId), JsonUtil.objectToJson(returnValue));
      return returnValue;
    }
  }
  ...
  public Object userSelectByUsername(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    String userJson =
        redisService.getString(String.format(RedisKeys.REDIS_USER_USERNAME_KEY, username));
    if (userJson != null) {
      // 带泛型转换, 这里如果不带泛型转换，会报错
      ...
      if (returnValue != null) {
        redisService.setString(
            String.format(RedisKeys.REDIS_USER_USERNAME_KEY, username),
            JsonUtil.objectToJson(returnValue));
      }
      return returnValue;
    }
  }
  ...
  public Object userSelectByToken(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    if (userJson != null) {
      ...
      if (returnValue != null) {
        redisService.setString(
            String.format(RedisKeys.REDIS_USER_TOKEN_KEY, token),
            JsonUtil.objectToJson(returnValue));
      }
      return returnValue;
    }
  ...
  public Object userSelectById(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    if (userJson != null) {
      ...
      if (returnValue != null) {
        redisService.setString(
            String.format(RedisKeys.REDIS_USER_ID_KEY, id), JsonUtil.objectToJson(returnValue));
      }
      return returnValue;
    }
  }
  ...
  static class RedisKeys {
    ...
    public static final String REDIS_TOPIC_VIEW_IP_ID_KEY =
        "pybbs_topic_view_ip_%s_topic_%s"; // 需要格式化字符串填充上ip地址跟话题id
