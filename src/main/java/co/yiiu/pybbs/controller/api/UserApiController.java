...
public class UserApiController extends BaseApiController {
  ...
  public Result topics(
      @PathVariable String username, @RequestParam(defaultValue = "1") Integer pageNo) {
    // 查询用户个人信息
    User user = userService.selectByUsername(username);
    // 查询用户的话题
  ...
  public Result comments(
    ...
    User user = userService.selectByUsername(username);
    // 查询用户参与的评论
    MyPage<Map<String, Object>> comments =
        commentService.selectByUserId(user.getId(), pageNo, null);
    Map<String, Object> map = new HashMap<>();
    map.put("user", user);
    map.put("comments", comments);
    return success(map);
  ...
  public Result collects(
    ...
    User user = userService.selectByUsername(username);
    // 查询用户参与的评论
    MyPage<Map<String, Object>> collects =
        collectService.selectByUserId(user.getId(), pageNo, null);
    Map<String, Object> map = new HashMap<>();
    map.put("user", user);
