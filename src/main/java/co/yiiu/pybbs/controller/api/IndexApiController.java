...
public class IndexApiController extends BaseApiController {
  ...
  @Resource private CookieUtil cookieUtil;
  ...
  // 根据标签名返回标签下话题
  @GetMapping("/tag/{name}")
  public Result topicsByTagName(
      @RequestParam(defaultValue = "1") Integer pageNo, @PathVariable String name) {
  ...
  private Result doUserStorage(HttpSession session, User user) {
    ...
    cookieUtil.setCookie(
        systemConfigService.selectAllConfig().get("cookie_name").toString(), user.getToken());
  ...
  public Result upload(
      @RequestParam("file") MultipartFile[] files, String type, HttpSession session) {
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      if (type.equalsIgnoreCase("video")) {
        long uploadVideoSizeLimit =
            Long.parseLong(
                systemConfigService.selectAllConfig().get("upload_video_size_limit").toString());
        if (size > uploadVideoSizeLimit * 1024 * 1024) {
          errors.add(
              "第[" + (i + 1) + "]个文件异常: " + "文件太大了，请上传文件大小在 " + uploadVideoSizeLimit + "MB 以内");
          continue;
        }
      } else {
        long uploadImageSizeLimit =
            Long.parseLong(
                systemConfigService.selectAllConfig().get("upload_image_size_limit").toString());
        if (size > uploadImageSizeLimit * 1024 * 1024) {
          errors.add(
              "第[" + (i + 1) + "]个文件异常: " + "文件太大了，请上传文件大小在 " + uploadImageSizeLimit + "MB 以内");
          continue;
        }
      }
