...
public class UserService implements IUserService {
  ...
  @Resource private Identicon identicon;
   ...
   * @param bio
   * @param website
   * @param needActiveEmail 是否需要发送激活邮件 新注册的用户都需要
    ...
    if (needActiveEmail) {
      ...
      new Thread(
              () -> {
                ...
                String content =
                    "如果不是你注册了%s，请忽略此邮件&nbsp;&nbsp;<a href='%s/active?email=%s&code=${code}'>点击激活</a>";
                codeService.sendEmail(
                    user.getId(),
                    email,
                    String.format(
                        title, systemConfigService.selectAllConfig().get("base_url").toString()),
                    String.format(
                        content,
