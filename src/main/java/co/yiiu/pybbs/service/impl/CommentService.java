...
public class CommentService implements ICommentService {
  ...
  @Resource @Lazy private IUserService userService;
  @Resource private INotificationService notificationService;
  @Resource private EmailService emailService;
  @Resource private TelegramBotService telegramBotService;
  ...
  public List<CommentsByTopic> selectByTopicId(Integer topicId) {
    ...
    for (CommentsByTopic commentsByTopic : commentsByTopics) {
      commentsByTopic.setContent(
          SensitiveWordUtil.replaceSensitiveWord(
              commentsByTopic.getContent(), "*", SensitiveWordUtil.MinMatchType));
    }
  ...
  public Comment insert(Comment comment, Topic topic, User user) {
    ...
    user.setScore(
        user.getScore()
            + Integer.parseInt(systemConfigService.selectAllConfig().get("create_comment_score")));
    ...
    if (comment.getCommentId() != null) {
      ...
      if (!user.getId().equals(targetComment.getUserId())) {
        notificationService.insert(
            user.getId(), targetComment.getUserId(), topic.getId(), "REPLY", comment.getContent());
                  ...
                  () ->
                      emailService.sendEmail(
                          ...
                          String.format(
                              emailContent,
                              comment.getContent(),
                              systemConfigService.selectAllConfig().get("base_url"),
                              topic.getId())))
              .start();
        }
      }
    ...
    if (!user.getId().equals(topic.getUserId())) {
      notificationService.insert(
          user.getId(), topic.getUserId(), topic.getId(), "COMMENT", comment.getContent());
      // 发送邮件通知
      ...
      if (systemConfigService.selectAllConfig().get("websocket").equals("1")) {
        MyWebSocket.emit(
            topic.getUserId(),
            new Message(
                "notifications", String.format(emailTitle, user.getUsername(), topic.getTitle())));
              ...
              if (systemConfigService.selectAllConfig().get("content_style").equals("MD")) {
                formatMessage =
                    String.format(
                        ...
                        user.getUsername(),
                        topic.getTitle(),
                        domain + "/topic/" + topic.getId(),
                        StringUtil.removeSpecialChar(comment.getContent()));
              } else {
                formatMessage =
                    String.format(
                        "%s 评论了话题 <a href=\"%s\">%s</a> 内容： %s",
                        user.getUsername(),
                        domain + "/topic/" + topic.getId(),
                        topic.getTitle(),
                        StringUtil.removeSpecialChar(comment.getContent()));
  ...
  public void delete(Comment comment) {
    if (comment != null) {
      ...
      user.setScore(
          user.getScore()
              - Integer.parseInt(
                  systemConfigService.selectAllConfig().get("delete_comment_score").toString()));
      userService.update(user);
      // 删除评论
  ...
  // 查询用户的评论
  @Override
  public MyPage<Map<String, Object>> selectByUserId(
    ...
    MyPage<Map<String, Object>> iPage =
        new MyPage<>(
            ...
            pageSize == null
                ? Integer.parseInt(
                    systemConfigService.selectAllConfig().get("page_size").toString())
                : pageSize);
  ...
  @Override
  public MyPage<Map<String, Object>> selectAllForAdmin(
